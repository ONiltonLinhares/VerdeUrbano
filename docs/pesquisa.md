# Pesquisa sobre o Problema e o Público


## 1. Informações relevantes sobre o problema

Grande parte da população brasileira vive em áreas urbanas. Segundo o **Censo Demográfico 2022 do IBGE**, aproximadamente **87,4% da população brasileira vive em áreas urbanas**.

Esse cenário aumenta a importância de espaços como parques, praças e outras áreas verdes dentro das cidades.

Entretanto, muitas pessoas possuem dificuldades para encontrar informações sobre esses locais, como:

- Localização;
- Acessibilidade;
- Horário de funcionamento;
- Estrutura disponível;
- Atividades que podem ser realizadas;

Além disso, muitas áreas verdes podem existir próximas aos moradores, mas não são conhecidas ou possuem poucas informações disponíveis na internet.

Segundo a **Organização Mundial da Saúde (OMS)**, áreas verdes urbanas podem contribuir para a saúde física e mental, incentivando atividades físicas, interação social e redução do estresse.

Dessa forma, o problema identificado pelo projeto não é apenas a falta de áreas verdes, mas também a **dificuldade de localizar e obter informações confiáveis sobre esses espaços**.

---

## 2. Necessidades e dificuldades dos usuários
O público do VerdeUrbano possui diferentes necessidades.

### Moradores de áreas urbanas
Precisam encontrar áreas verdes próximas de suas residências e conhecer as características desses locais antes de visitá-los.

### Idosos
Podem precisar de informações relacionadas a:

- Bancos para descanso;
- Banheiros;
- Caminhos adequados;
- Rampas;
- Acessibilidade.

### Famílias com crianças
Precisam encontrar locais que ofereçam:

- Segurança;
- Áreas infantis;
- Espaços para lazer;
- Estrutura adequada para crianças.

### Pessoas com deficiência ou mobilidade reduzida
Necessitam saber antecipadamente se o local possui:

- Rampas;
- Calçadas acessíveis;
- Caminhos adequados;
- Banheiros acessíveis.

Dados do **IBGE** mostram que apenas uma parcela das vias urbanas brasileiras possui estruturas adequadas de acessibilidade, tornando esse tipo de informação importante para o aplicativo.

### Profissionais ligados à saúde
Profissionais como psicólogos podem utilizar informações sobre áreas verdes para recomendar atividades relacionadas ao contato com a natureza, relaxamento e bem-estar.

---

## 3. Dados que podem influenciar o aplicativo
A pesquisa mostrou que apenas apresentar a localização de uma área verde pode não ser suficiente.

O VerdeUrbano deve disponibilizar outras informações importantes.

### Localização
O aplicativo poderá utilizar mapas para mostrar parques, praças e outras áreas verdes próximas ao usuário.

### Distância
O usuário poderá visualizar quais locais estão mais próximos.

### Acessibilidade
Cada local poderá possuir informações sobre:

- Rampas;
- Calçadas;
- Caminhos acessíveis;
- Banheiros adaptados.

### Segurança
O aplicativo poderá apresentar informações como:

- Iluminação;
- Horários de funcionamento;
- Avaliações dos usuários;
- Condições do local.

### Atividades disponíveis
Os locais poderão ser classificados de acordo com atividades como:

- Corrida;
- Caminhada;
- Exercícios;
- Recreação infantil;
- Descanso;
- Contato com a natureza.

### Estrutura
O aplicativo poderá informar a existência de:

- Banheiros;
- Bancos;
- Bebedouros;
- Parquinhos;
- Equipamentos esportivos;
- Estacionamento.

### Avaliações dos usuários
Os próprios usuários poderão compartilhar informações sobre:

- Conservação;
- Segurança;
- Limpeza;
- Acessibilidade;
- Qualidade do espaço.

### Funcionamento offline
Como algumas áreas podem apresentar sinal de internet limitado, o aplicativo poderá permitir o download de mapas e informações para consulta offline.

---

## 4. Fontes utilizadas

### IBGE — Instituto Brasileiro de Geografia e Estatística

Dados do Censo Demográfico 2022 sobre população urbana e infraestrutura das cidades brasileiras.

- https://educa.ibge.gov.br/criancas/voce-sabia/22584-populacao-urbana-e-rural.html

- https://www.ibge.gov.br/geociencias/organizacao-do-territorio/tipologias-do-territorio/24702-caracteristicas-urbanisticas-do-entorno-dos-domicilios.html

### Organização Mundial da Saúde — OMS

Informações sobre os benefícios das áreas verdes urbanas para a saúde e qualidade de vida.

- https://www.who.int/europe/publications/i/item/WHO-EURO-2016-3352-43111-60341

### Organização das Nações Unidas — ONU

Objetivo de Desenvolvimento Sustentável 11, relacionado à criação de cidades inclusivas, seguras, sustentáveis e com acesso a espaços verdes.

- https://sdgs.un.org/goals/goal11

### Organização Mundial da Saúde — Espaços verdes e saúde mental

Estudos sobre a relação entre espaços verdes, saúde mental e bem-estar.

- https://www.who.int/europe/publications/i/item/9789289055666

---
